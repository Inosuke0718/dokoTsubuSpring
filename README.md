...
Spring Boot版「どこつぶ（dokoTsubuSpring）」について、公式が公開しているソースコードには一部不足・誤りがあります。
2026年5月27日時点では、15章の「どこつぶ」関連ソースコードに修正が必要な箇所があります。

- 公式リポジトリ  
  https://github.com/miyabilink/sukkiri-servlet5-codes/tree/main/dokoTsubuSpring

- 公式リポジトリ 15章  
  https://github.com/miyabilink/sukkiri-servlet5-codes/tree/main/15%E7%AB%A0

そのため、学習時は以下の訓練校側で用意したリポジトリを参考にするのがおすすめです。
...
|                  | `IndexController.java`      | **【※教科書での修正指示がない】**`/` へのアクセスを `src/main/webapp/index.jsp` にフォワードさせたい場合にこのControllerを新規作成。 | -                                 |
...
| **設定/その他**  | `pom.xml`                   | **【※Web付録にて指示】**組み込みTomcatでJSPを実行するための`tomcat-embed-jasper`と、JSTLを使用するための依存関係を追加。             | Web付録:ビューにJSPを使用する設定 |
|                  | `application.properties`    | **【※Web付録にて指示】**Controllerが返すビュー名を`WEB-INF/jsp/`配下のJSPファイルにマッピングするビューリゾルバ設定を追加。          | Web付録:ビューにJSPを使用する設定 |

---

## 3. 教科書に載っている以外で注意が必要なところ（詳細な理由）
...
### 1. `http://localhost:8080/`アクセス時のエラー対応
